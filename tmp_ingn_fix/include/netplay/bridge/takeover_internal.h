...
struct SharedBlock
{
    ...
    // tick hook treats as an immediate disconnect.  An RNG-only mismatch
    // can be latent for a long interval and only become gameplay-visible
    // when a later mechanic consumes randomness, so the native warning is
    // evidence to capture rather than a safe reason to abort or reseed.
