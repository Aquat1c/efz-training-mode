...
void NoteConsolePromptLine(const std::string& text)
{
    ...
    // Plain "Desync detected" is Revival's one-shot Sync-record inequality
    // warning. Stock Revival keeps comparing after printing it. Process all
    // specific fatal protocol/transport phrases above first so a combined
    // line can never be downgraded by this broader match.
