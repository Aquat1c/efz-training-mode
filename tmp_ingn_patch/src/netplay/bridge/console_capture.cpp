...
void NoteConsolePromptLine(const std::string& text)
{
    ...
    // continues and later frames can log "Checked N" again), and RNG-only
    // transients from rollback prediction reconverge within a few frames.
    // Publish it on the provisional channel instead of consoleErrorSerial so
    // the tick hook does not tear the session down.  Persistent gameplay-state
    // desyncs are confirmed separately by desync_monitor (mod/mod) with a
    // >1-second mismatch-streak threshold.
