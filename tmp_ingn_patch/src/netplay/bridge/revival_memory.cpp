...
static int __fastcall OurPerFrameTickHook(void* exeThis, void* /*edx*/)
{
    ...
    // running after the one-shot warning, and RNG-only Sync mismatches from
    // rollback prediction reconverge within a few frames
    ...
    // be diffed for reconvergence.  Persistent gameplay-state desyncs are
    // confirmed (and forensically dumped) by desync_monitor's >1s
    // mismatch-streak policy; transport/protocol failures still use the
        ...
        if (newDesyncWarning || trailSampleDue)
        {
            ...
            if (newDesyncWarning)
            {
                ...
                mod::Log(
                    ...
                    "alive (stock-parity); desync_monitor owns confirmation",
            ...
            else
            {
                ...
                s_desyncWarnNextSampleTick += 1000;
                mod::Log(
    ...
    if (!preTickDisconnect)
    {
        ...
        if (revivalBatchZeroFrameDiag)
        {
                ...
                && revivalTickAfter.patchState == 0)
            {
                ...
                // A/B axis.  [Others] EagerZeroFrameGraphicsRestore=0 gives
                // stock parity; terminal-recovery restores are separate and
                // unaffected.
                if (netplay::mod_settings::IsEagerZeroFrameGraphicsRestoreEnabled())
                {
                ...
                {
                    mod::Log(
                        "REVIVAL_ZERO_FRAME_PATCH_LEAK_LEFT_ALONE reason=stock_parity_setting "
                        "currentFrame=%d",
                        revivalTickBefore.currentFrame);
                }
            }
