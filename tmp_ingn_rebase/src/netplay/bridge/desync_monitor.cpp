    ...
    int entityData)
{
    ...
    if (tracing)
    {
        __try
        {
            slot = *reinterpret_cast<const volatile uint16_t*>(
                reinterpret_cast<uintptr_t>(gameSys) + kEffectAllocCursorOffset);
            requestedBehavior = *reinterpret_cast<const volatile uint16_t*>(
                entityData + 4);
        }
        __except (EXCEPTION_EXECUTE_HANDLER)
        {
...
bool InstallType47TraceHooks()
{
        ...
        || createPass != MH_OK || createOne != MH_OK)
    {
        ...
        if (createInit == MH_OK)
            (void)MH_RemoveHook(reinterpret_cast<void*>(kInitializeEffectAddr));
        if (createClear == MH_OK)
            (void)MH_RemoveHook(reinterpret_cast<void*>(kClearEffectAddr));
        if (createPass == MH_OK)
            (void)MH_RemoveHook(reinterpret_cast<void*>(kProcessEffectsAddr));
        if (createOne == MH_OK)
            (void)MH_RemoveHook(reinterpret_cast<void*>(kProcessEffectAddr));
        g_type47HooksFailed = true;
        mod::Log(
            "TYPE47_TRACE: hook creation failed init=%d clear=%d pass=%d one=%d",
    ...
    {
        (void)MH_DisableHook(reinterpret_cast<void*>(kInitializeEffectAddr));
        (void)MH_DisableHook(reinterpret_cast<void*>(kClearEffectAddr));
        (void)MH_DisableHook(reinterpret_cast<void*>(kProcessEffectsAddr));
        (void)MH_DisableHook(reinterpret_cast<void*>(kProcessEffectAddr));
        (void)MH_RemoveHook(reinterpret_cast<void*>(kInitializeEffectAddr));
        (void)MH_RemoveHook(reinterpret_cast<void*>(kClearEffectAddr));
        (void)MH_RemoveHook(reinterpret_cast<void*>(kProcessEffectsAddr));
        (void)MH_RemoveHook(reinterpret_cast<void*>(kProcessEffectAddr));
        g_type47HooksFailed = true;
        mod::Log(
            "TYPE47_TRACE: hook enable failed init=%d clear=%d pass=%d one=%d",
...
bool DisableAndRemoveType47TraceHooks()
{
    ...
    mod::Log(
        "TYPE47_TRACE: hooks quiesced disabled=%d removed=%d",
        disabled ? 1 : 0,
        removed ? 1 : 0);
    return disabled;
}
...
bool WriteType47TraceCsv(const std::string& dumpDir)
{
    ...
    const LONG end = sealedEnd > 0 && sealedEnd < currentEnd
        ? sealedEnd
        : currentEnd;
...
bool WriteDesyncDump()
{
    ...
    if (report != INVALID_HANDLE_VALUE)
    {
        ...
        AppendText(report,
            "type47 limitation: this build brackets EFZ effect updates but has no "
            "profile-verified Revival snapshot SAVE/LOAD hooks. A first unequal "
    ...
    const char* nickname)
{
    ...
    // The observer exists only for explicitly traced host/join sessions.
    // Recreate the owned hooks after prior evidence has been finalized; the
    // hot hook bodies remain disabled until the negotiated future window.
...
void RecordFrameTick(uintptr_t sessionPtr, int frame, int commitFrame)
{
    ...
    else if (g_postEvidenceFrameCount < kPostEvidenceDepth)
    {
        std::memcpy(
            ...
            &staging,
            sizeof(staging));
        if (g_postEvidenceFrameCount == kPostEvidenceDepth)
        {
            // Freeze the detailed ring after the bounded causal post-window
            // so a long-running match cannot overwrite the onset.
            InterlockedExchange(&g_type47TraceSealed, 1);
            InterlockedExchange(&g_type47TraceEnabled, 0);
        }
    }
